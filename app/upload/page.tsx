...
function parseSTLVolume(buffer: ArrayBuffer) {
  ...
  if (buffer.byteLength < 84) return { volume: 0, sizeX: 0, sizeY: 0, sizeZ: 0, hasError: true }
  ...
  for (let i = 0; i < faceCount; i++) {
    ...
    const v1x = dv.getFloat32(offset + 12, true), v1y = dv.getFloat32(offset + 16, true), v1z = dv.getFloat32(offset + 20, true)
    const v2x = dv.getFloat32(offset + 24, true), v2y = dv.getFloat32(offset + 28, true), v2z = dv.getFloat32(offset + 32, true)
    const v3x = dv.getFloat32(offset + 36, true), v3y = dv.getFloat32(offset + 40, true), v3z = dv.getFloat32(offset + 44, true)
    vol += (-v3x * v2y * v1z + v2x * v3y * v1z + v3x * v1y * v2z - v1x * v3y * v2z - v2x * v1y * v3z + v1x * v2y * v3z) / 6
    const xs = [v1x, v2x, v3x], ys = [v1y, v2y, v3y], zs = [v1z, v2z, v3z]
  ...
  return {
    volume: Math.abs(vol) / 1000,
    sizeX: Math.round((maxX - minX) * 10) / 10,
    sizeY: Math.round((maxY - minY) * 10) / 10,
    sizeZ: Math.round((maxZ - minZ) * 10) / 10,
    hasError: false
  }
}

// ─── PROMPTPAY HELPER ───
function buildPromptPayQR(id: string, amount: number) {
  const target = id.replace(/-/g, "")
  const tag29 = target.length === 10 ? "0066" + target.substring(1) : "0013" + target
  const payload = [
    "000201", "010211", "29370016A000000677010111" + tag29, "5802TH", "5303764",
    "54" + amount.toFixed(2).length.toString().padStart(2, "0") + amount.toFixed(2), "6304"
  ].join("")
  return payload // In real case, add CRC-16
}
...
export default function UploadPage() {
  ...
  const [infill, setInfill] = useState<InfillLevel>(20)
  const [layerHeight, setLayerHeight] = useState<LayerHeight>(0.2)
  ...
  const handleCheckout = () => { if (result) setShowInfoModal(true) }
  ...
  const handleConfirmOrder = async () => {
    ...
    const order = { id, createdAt: new Date().toISOString(), status: 'pending', customer: { name: custName, email: custEmail, phone: custPhone, address: custAddr }, items: [{ name: model?.name, technology, material, color, quantity, price: result?.totalPrice }] }
    ...
    const bodyText = `คำสั่งซื้อใหม่\n\nORDER: ${num}\nวันที่: ${new Date().toLocaleString('th-TH')}\n\n📂 ไฟล์: ${model?.name}\n⚙️ วัสดุ: ${material} | ${technology}\n🎨 สี: ${color}\n📐 Layer: ${layerHeight}mm | Infill: ${infill}%\n🔢 จำนวน: ${quantity} ชิ้น\n\n💰 ราคารวม: ${result?.totalPrice} THB\n⏱ เวลาพิมพ์: ${formatTime(result?.printTimeMin || 0)}\n⚖️ น้ำหนัก: ${result?.weightG}g\n\n👤 ${custName}\n📧 ${custEmail}\n📞 ${custPhone}\n📍 ${custAddr}\n\n💬 หมายเหตุ: ${notes || '—'}`

    window.location.href = `mailto:order@pb3dprint.com?cc=${encodeURIComponent(custEmail)}&subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(bodyText)}`
    setShowInfoModal(false)
    setShowQR(true)
  }

  const handlePaymentDone = () => { setShowQR(false); setShowSuccess(true) }
          ...
          <div className="overflow-y-auto flex-1 pb-10 custom-scrollbar">
              ...
              <div className="flex gap-2 mt-3">
                {(["FDM", "RESIN"] as Technology[]).map(t => (
                  <button key={t} onClick={() => setTechnology(t)}
                    className={`flex-1 py-2 rounded-xl border-2 transition font-bold text-xs ${technology === t ? "border-violet-500 bg-violet-50 text-violet-700" : "border-gray-100 text-gray-400 hover:border-gray-200"}`}>{t}</button>
                ))}
              </div>
            </Section>
            ...
            <Section title="Detail & Infill">
              <div className="space-y-4 mt-3">
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase">Layer Height</label>
                  <div className="flex gap-2 mt-1">
                    {[0.1, 0.2, 0.3].map(v => (
                      <button key={v} onClick={() => setLayerHeight(v as any)} className={`flex-1 py-1.5 rounded-lg border text-xs transition ${layerHeight === v ? "bg-violet-600 border-violet-600 text-white" : "border-gray-100 text-gray-600 hover:border-gray-300"}`}>{v}mm</button>
                    ))}
                  </div>
                </div>
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase">Infill Density ({infill}%)</label>
                  <input type="range" min="10" max="100" step="10" value={infill} onChange={e => setInfill(Number(e.target.value) as any)} className="w-full h-1.5 bg-gray-100 rounded-lg appearance-none cursor-pointer mt-2 accent-violet-600" />
                </div>
              </div>
            </Section>
            <Section title="Color">
        ...
        <div className="fixed inset-0 bg-black/60 z-[200] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="bg-[#1e1e2e] px-6 py-5 flex justify-between items-center">
              <h3 className="text-white font-black text-lg">📝 ข้อมูลการจัดส่ง</h3>
              <button onClick={() => setShowInfoModal(false)} className="text-gray-400 hover:text-white text-xl">✕</button>
            </div>
